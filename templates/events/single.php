...
if ((int) $post->post_author == (int) get_current_user_id() && get_current_user_id() != 0) :
    ?>
        <a href="<?php echo add_query_arg('event', get_post_field( 'post_name', get_post() ), get_home_url().'/edit-event/')     ; ?>"><?php echo __('Edit', THEME); ?></a>
        <a class="remove" data-remove="<?php echo __('Do you really want to remove this item?');?>" href="<?php echo add_query_arg(array ('event'=>get_post_field( 'post_name', get_post() ), 'remove' => 1 ), get_home_url().'/edit-event/')     ; ?>"><?php echo __('Remove', THEME); ?></a>
    <?php
endif;
